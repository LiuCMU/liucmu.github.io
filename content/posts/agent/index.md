...
- level 1: the connected problem solver. This is the minimal agent that contains the three core components mentioned above. These typically can be implemented in a few lines of code with common agent develop kit (such as Google ADK, OpenAI SDK, Anthropic SDK, LangChain, LallaIndex, etc.). You probably saw them in quick start examples from many of these SDKs. The agent comes with a simbple context iformtion and some built-in tools, orchestration is largely handled by the built-in SDK.
