...
Large language models (LLMs) are more powerful when equipped with the capability to interact with the outside world, and an agent is the way to let an LLM interact with the outside world. In essence, an LLM predicts the next word or token based on existing words in a sentence or paragraph. This is a classical statistical problem. With recent innovation in language model architectures and computing hardware, both the model size and training data can scale to a much larger size. Current popular LLMs are believed to have been trained on a large portion of available information on the internet, and contain hundreds of billions to trillions of parameters, hence the term "large" language model. Increasing the model size and training data leads to very good advantages in model behavior. By **summarizing all information patterns** as model parameters, an LLM now essentially is a lossy compressed storage of all information on the internet. It understands the grammar of most languages and the world logic embedded in the languages. With a little bit of post-training (for example, supervised instruction tuning<sup>[1](#ref-1)</sup>, reinforcement learning from human feedback (RLHF)<sup>[2](#ref-2)</sup>, direct preference optimization (DPO)<sup>[3](#ref-3)</sup>, and constitutional AI<sup>[4](#ref-4)</sup>), they can now answer many questions reasonably well. Apparently, LLMs are like a super brain but are limited to information up to a certain date, and without hands for interacting with the outside world. Consequently, people begin to explore ways to let the LLM get up-to-date information and act in the real world.
...
- LLM: this is like the **brain** of the system. For agentic systems, the reasoning ability is the key to navigate complex and multi-step problems. A good reasoning model already has the chain-of-thought (CoT)<sup>[5](#ref-5)</sup> and ReAct (reasoning and acting)<sup>[6](#ref-6)</sup> ability built-in, thus simplifying the orchestration layer burden.
...
- level 0: the core reasoning system, i.e. the LLM operates in isolation.
- level 1: the connected problem solver. This is the minimal agent that contains the three core components mentioned above. These typically can be implemented in a few lines of code with a common agent development kit (such as Google ADK<sup>[8](#ref-8)</sup>, OpenAI Agents SDK<sup>[9](#ref-9)</sup>, Anthropic Claude Agent SDK<sup>[10](#ref-10)</sup>, LangChain<sup>[11](#ref-11)</sup>, LlamaIndex<sup>[12](#ref-12)</sup>, etc.). You probably saw them in the quick-start examples from many of these SDKs.
- level 2: a strategic problem solver. At this level, customized context information and tools are provided. Most context engineering, customized tool building and logic are built-in. At this level, the agent is like a domain expert that would outperform a general model like GPT without customized tools or context. The agent comes with the minimal necessary context information and some tools; orchestration is largely handled by the built-in SDK and hand-crafted logic. In my practical agent development experience, I find simple agents with the Anthropic/OpenAI SDK often achieve better or the same performance as a complex agent with heavy developer logic, under the same context information — a preference for simplicity also emphasized in Anthropic's *Building Effective Agents*<sup>[13](#ref-13)</sup>.
- level 3: a collaborative multi-agent system. At this level, multiple agents collaborate to achieve a goal. Agents are able to communicate with each other to assign tasks and receive the results. One common arrangement is that one agent acts as the orchestrator, and a few other agents are like different domain experts solving different problems. The larger, complex goal is decomposed into smaller goals by the orchestrator; each agent solves tasks for its role and reports the results to the orchestrator; the orchestrator integrates the results into one complete answer, and spawns new sub-agents as needed.
- level 4: a self-evolving system. The agent uses a multi-agent system to solve a long-running task, gets some initial results, understands the gap between the ideal solution and its current capability limits, then is able to create new context, skills, tools or even agents to fill the gap and achieve its target. This can even be integrated with lab automation to form a closed loop in scientific and industrial research.
...
Tools empower LLMs to do more tasks. Trained LLMs become experts in general domains, but they lack information on the current state of the world and the ability to take actions. Tools serve that purpose. Apparently, a tool either enables the LLM to get more information or allows it to take actions in the real world. Some informational tool examples include a web search tool that enables the LLM to get up-to-date information, or a tool that can query an internal database. An action tool example includes a code execution sandbox for data analysis. The boundary between informational and action tools is fuzzy, because, for example, some tools take actions in order to get information. In essence, it's all about **gathering information and transforming information, until the task is solved**. The gathered information forms a context that is tailored to a specific task and aligns with the general patterns that the LLM knows from its training process; then the LLM outputs the next tokens based on the surrounding context information. The tokens become context for the next round of exploration. This process is iterated until a solution is output. This propose-act-observe loop (Figure 1) is the core of an agent<sup>[6](#ref-6), [13](#ref-13)</sup>.
...
Tools take many formats. Actually, the mainstream agent development kits mentioned above all come with a suite of powerful built-in tools, like web search. Customized tools can be command-line tools for certain software — for example, the AWS CLI for exploring cloud status; Python functions for achieving a specific task; or any other API that can be consumed to do a specific job.

A common way of integrating models and tools is called the Model Context Protocol (MCP)<sup>[16](#ref-16)</sup>. MCP aims to serve as the universal interface between AI applications and the vast world of external tools and data. MCP contains the following components:
...
MCP is more or less a thin wrapper with good documentation over existing tools or APIs. For many local use cases, a well-documented tool and an MCP behave similarly. But the existence of remote MCP servers makes it easier to share a capability across multiple agents, fostering a reusable ecosystem.

Clear documentation is the key to building good tools, including MCPs. We need a clear definition of the input meaning and types, output meaning and types, a docstring on what the tool does, informative names, etc. Everything is pretty much the same as we have in the [PEP 8 guidelines](https://peps.python.org/pep-0008/) for writing clear code.
...
LLMs are stateless in isolation, meaning they don't remember anything beyond what is fed into them as input. A stateful and personal chat experience in commercial chatbots comes from context engineering. Let's define a turn as a user input and the corresponding LLM response, and a session as a complete chat history containing many turns between the LLM and user. In agentic applications, it appears that the LLM knows the chat history within a turn; that is because all the questions and answers are fed to the LLM before each turn starts. The mechanism enabling LLMs to remember chat history in a session is called short-term memory or session memory. To enable session memory so that the LLM knows all necessary context in a chat session, the most common ways are a sliding window or asynchronous summary. For a sliding window, the orchestration layer keeps only the most recent k turns. This is simple and cheap, but loses relevant information in a long chat. Recursive summary uses an LLM to summarize the chat history as the conversation goes, so each question and answer in the chat history is stored in a brief format as context for the LLM to answer the current question. The summary process typically runs in the background. This method is able to keep more relevant information, but also requires more compute. When we start a new session, it appears the LLM has some general persona tailored to the user or project; this is because there is a factsheet preloaded before the start of each session. The mechanism for managing an LLM persona and tailoring it to a user or project is called long-term memory. Long-term memory is typically general factual or procedural information extracted from short-term memories. It is saved to benefit future conversations.

Both short-term and long-term memory are just text files, like a README; the tricky part is how to curate, retrieve, use and update this information, which collectively is called prompt engineering and context engineering. Let's take an example to demonstrate the ideas. Imagine that, in an agentic application, in a turn, the user asks a question. What the LLM sees is probably something like the following. The LLM sees much more than just the user question — many other pieces of relevant information that are useful to answer the question. That's why the token consumption is always much higher than the chat history alone.
...
As with any ML system, accuracy is the most important metric for evaluating agent performance. Another important metric is the efficiency of the agent. Efficiency can be measured by two aspects: the number of tokens used to answer a question, and the trajectory of the agent to reach the answer. The number of tokens used is a direct measure of the cost of using the agent, and the trajectory is a measure of how efficient the agent is in reaching the answer. A good agent should be able to reach the answer with minimal tokens and in a short trajectory.

Now on to designing a benchmark for the accuracy part. The non-deterministic nature of LLMs requires more careful attention in designing the question-answer pairs for an agent, and the evaluation is typically a combination of fixed metrics, LLM-as-a-judge and human review. For questions where the answers are free text, most metrics from conventional linguistic evaluation can be used — for example BLEU<sup>[30](#ref-30)</sup> (a precision-oriented n-gram metric from machine translation) and ROUGE<sup>[31](#ref-31)</sup> (a recall-oriented metric from summarization). Note that these n-gram overlap metrics correlate weakly with human judgment on open-ended answers, so semantic metrics and LLM-as-a-judge (with a detailed rubric) are often preferred. Exploiting questions whose answers are numerical or multiple choice is a good approach, because it allows deterministic evaluation. For example, frame the question as "how many ..." or "list ...". Confining the questions up to a specific date is helpful in case the dataset changes over time; for example, a question can be "What is the number of ... up to 2026-08-11?". It is also helpful to evaluate the agent's ability to respond to negative questions. For example, if the question is ambiguous, or if the question does not contain all the information needed to answer it, does the agent know to ask the user to clarify, or does it just make implicit assumptions? If a question is out of scope of its accessible data and tools, does the agent know to refuse and not make up an answer? Common tools for logging agent traces and evaluating agents include Langfuse (open source)<sup>[32](#ref-32)</sup>, LangSmith (commercial)<sup>[33](#ref-33)</sup>, and Weights & Biases Weave (open-source SDK within a commercial platform)<sup>[34](#ref-34)</sup>.

Agent security is worth equal attention to agent capability. Since an agent can take actions in the outside world, it can potentially take wrong and even harmful actions. These are not hypothetical: during red-team evaluations, OpenAI's o1 exploited a misconfigured Docker daemon to complete a capture-the-flag task in an unintended way<sup>[35](#ref-35)</sup>; reasoning models have been observed manipulating the game state to win at chess rather than playing fairly<sup>[36](#ref-36)</sup>; and in stress tests, leading models chose harmful actions such as blackmail or data leaks when their goals were threatened<sup>[37](#ref-37)</sup>. Attackers can also hijack tool-using agents through indirect prompt injection — hiding instructions in a web page or document the agent reads<sup>[38](#ref-38)</sup>. Not only should we carefully design the agent logic to minimize the risk of bad actions, but we should also take full advantage of existing software infrastructure security features to safeguard the agent's behavior. Deterministic guardrails are necessary for agent security. For example, a hardcoded rule that an explicit user confirmation is needed to call a specific tool. Relying on the LLM's judgment can be a supplement, but it can be manipulated by techniques like prompt injection. An example list of common guardrails from traditional software that can be used includes:
...
With all the components ready, we need to containerize the agent application; then this container can be deployed via a common cloud service like AWS. Many cloud services take care of the process of scaling to many users if needed — for example, AWS EKS. It is worth noting this is just a general way of deploying an agent application for broad usage. If your agent is developed with a specific agent development kit, like Google ADK, many of them provide a managed or one-click-style deploy to their own agent runtime (for example, Vertex AI Agent Engine)<sup>[39](#ref-39)</sup>. This is even more convenient, but is very often limited to a specific provider.
...
## Conclusions
With this content, hopefully you get a basic understanding of the fundamentals of agent development, and are ready either to try or build an agent yourself, and empower your daily work. Agents are good at tasks with clear goals and well-defined workflows. This is determined by the nature of LLMs, and is helpful in using agents more efficiently. Much of the framework in this post draws on Google's *Introduction to Agents* whitepaper<sup>[7](#ref-7)</sup> from the 5-Day Gen AI Intensive course<sup>[40](#ref-40)</sup>.
